...
export function asyncReap (options, done = () => {}) {
  ...
  if (isWindows) {
    childProcess.execFile('node', [path.join(WHEREAMI, './lib/reap-worker.mjs'), options.path, options.ttl], execDone)
  } else {
    childProcess.execFile(path.join(WHEREAMI, './lib/reap-worker.mjs'), [options.path, options.ttl], execDone)
  }
}
