    ...
    fs.readFile(filePath, isSecret(options.secret) && !options.encryptEncoding ? null : options.encoding, function readCallback (err, data) {
      if (!err) {
        ...
        try {
          ...
          return fs.remove(filePath, function (removeError) {
            if (removeError) {
...
export function set (sessionId, session, options, done = () => {}) {
  try {
    ...
    writeFileAtomic(filePath, json, function (err) {
      return (
...
export function encrypt (options, data, sessionId) {
  let ciphertext = null
...
export function decrypt (options, data, sessionId) {
  let plaintext = null
